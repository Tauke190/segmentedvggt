...
def unproject_depth_map_to_segmented_point_map(
    depth_map: np.ndarray, extrinsics_cam: np.ndarray, intrinsics_cam: np.ndarray
) -> np.ndarray:
    """
    Unproject a batch of depth maps (optionally with segmentation mask) to 3D world coordinates.

    Args:
        depth_map (np.ndarray): Batch of depth maps of shape (S, H, W, 1), (S, H, W), or (S, H, W, 2)
                               If (S, H, W, 2): depth_map[..., 0] is depth, depth_map[..., 1] is segmentation mask.
        extrinsics_cam (np.ndarray): Batch of camera extrinsic matrices of shape (S, 3, 4)
        intrinsics_cam (np.ndarray): Batch of camera intrinsic matrices of shape (S, 3, 3)

    Returns:
        np.ndarray: Batch of 3D world coordinates of shape (S, H, W, 4), where the last dimension is (x, y, z, mask)
    """
    ...
    for frame_idx in range(depth_map.shape[0]):
        # Handle (H, W, 2) or (H, W, 1) or (H, W)
        frame = depth_map[frame_idx]
        if frame.ndim == 3 and frame.shape[-1] == 2:
            depth = frame[..., 0]
            mask = frame[..., 1]
        elif frame.ndim == 3 and frame.shape[-1] == 1:
            depth = frame.squeeze(-1)
        ...
        cur_world_points, _, _ = depth_to_world_coords_points(
            depth, extrinsics_cam[frame_idx], intrinsics_cam[frame_idx]
        )  # (H, W, 3)

        # Concatenate mask as the 4th channel
        cur_world_points_with_mask = np.concatenate(
            [cur_world_points, mask[..., None]], axis=-1
        )  # (H, W, 4)

        world_points_list.append(cur_world_points_with_mask)
    world_points_array = np.stack(world_points_list, axis=0)  # (S, H, W, 4)
